            ...
            <div className="requests relative">
              {requests?.length && requests?.length > 0 ? (
                <div
                  ...
                  className="absolute bg-gradient-to-t from-pink-500 to-violet-500 text-white font-bold text-center  p-1 -top-5 -right-2 text-sm"
                >
                  {requests?.length}
                </div>
              ) : null}
              ...
              requests && requests.length > 0 ? (
                <Card className="bg-gray-400 w-max p-5 -translate-x-full p-3 absolute">
                  {requests.map((item) => {
                    ...
                    return (
                      <div
                        key={item._id}
                        className="bg-gray-100 my-1  transition-all  cursor-pointer flex item-center p-3"
                      >
                        <img
                          width={"40px"}
                        ...
                        <div className="flex flex-col">
                          ...
                          <button
                            onClick={() => acceptRequest(item?.from, item?.to)}
                            className="bg-gradient-to-r border-none  text-sm w-full  from-pink-800 font-bold to-purple-600 text-white hover:bg-gradient-to-bl transition py-1 px-4  rounded-full "
                          >
                            Accept
                          </button>
                        </div>
                ...
                <Card className="bg-gray-400 w-max -translate-x-full p-3 absolute">
                  <div className="bg-gray-100 my-1 hover:scale-110 transition-all hover:shadow cursor-pointer flex item-center p-3">
                    <h5 className="font-bold">No new requests</h5>
                  </div>
                </Card>
              )
            ) : null}
